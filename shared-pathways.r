...
allc <- allc[allc$padj < 0.05,]

#Adding a column for shared/unique pathways:
allc <- allc %>% mutate(group = outcome, outcome = NULL) 
allc$shared <- allc$group
allc[allc$pathway %in% intersect(allc[allc$group == "deceased",]$pathway, allc[allc$group == "survived",]$pathway),]$shared <- "shared"

allc$shared <- factor(allc$shared, levels = c("shared","deceased", "survived"))
...
allc$category[allc$pathway == "HYPOXIA" | allc$pathway == "UNFOLDED PROTEIN RESPONSE" | allc$pathway == "EPITHELIAL MESENCHYMAL TRANSITION" | allc$pathway == "APICAL JUNCTION"] <- "other"
allc$category <- factor(allc$category, levels = c("immune", "signaling", "metabolism", "proliferation", "other"))
...
#Plotting bubble plots, divided by progress, including the shared pathways:
immune <- ggplot(transform(allc[allc$category == "immune",]),
                 aes(x = NES, y = reorder(pathway,NES), color = group, size = size)) +
  geom_point(alpha=0.7) +
  facet_grid(vars(category), labeller = labeller(category = labels), drop = TRUE, space = "free") +
  ylab("Pathways") +
...
proliferation <- ggplot(transform(allc[allc$category == "proliferation",]),
                        aes(x = NES, y = reorder(pathway,NES), color = group, size = size)) +
  geom_point(alpha=0.7) +  
  facet_grid(vars(category), labeller = labeller(category = labels), drop = TRUE, space = "free") +
  theme(strip.text.x = element_blank()) +
...
signaling <- ggplot(transform(allc[allc$category == "signaling",]),
                      aes(x = NES, y = reorder(pathway,NES), color = group, size = size)) +
  geom_point(alpha=0.7) +  
  facet_grid(vars(category), labeller = labeller(category = labels), drop = TRUE, space = "free") +
  theme(strip.text.x = element_blank()) +
...
metabolism <- ggplot(transform(allc[allc$category == "metabolism",]),
                     aes(x = NES, y = reorder(pathway,NES), color = group, size = size)) +
  geom_point(alpha=0.7) +  
  facet_grid(vars(category), labeller = labeller(category = labels), drop = TRUE, space = "free") +
  theme(strip.text.x = element_blank()) +
...
other <- ggplot(transform(allc[allc$category == "other",]),
                     aes(x = NES, y = reorder(pathway,NES), color = group, size = size)) +
  geom_point(alpha=0.7) +  
  facet_grid(vars(category), labeller = labeller(category = labels), drop = TRUE, space = "free") +
  theme(strip.text.x = element_blank()) +
...
tiff(filename = "graphs/pathways-category-bubble-critical.tiff",
     width = 15, height = 10, units = "in", res = 300)
bubble
dev.off()
